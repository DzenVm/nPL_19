...
przyjaciele językowi, organizacja zajęć, cennik orientacyjny, FAQ), a
`/metoda`, `/poziomy` i `/sprawdz-poziom` rozwijają wybrane wątki na osobnych
podstronach.
...
- **Next.js 16** (App Router) + React 19 + TypeScript, z włączonym
  **Cache Components** (`cacheComponents: true`) — strona główna jest w
  większości statycznym shellem HTML, a jedyną prawdziwą dziurą dynamiczną
  jest widżet „najbliższy start grupy”, liczony z bieżącej daty serwera w
  strefie `Europe/Warsaw` przez `connection()` + `<Suspense>`. To nie
  odliczanie promocyjne — grupy realnie startują w każdy poniedziałek.
...
do przeglądarki Chromium (w tym środowisku already skonfigurowany przez
...
src/content/               # typizowana treść (poziomy, metoda, pytania, cennik, quiz…)
...
- Brak fałszywych opinii, ocen czy liczników uczniów; brak fałszywego
  odliczania promocji — widżet startu grupy pokazuje realną datę najbliższego
  poniedziałku, nie malejącą pulę „ostatnich miejsc”.
- Cennik oznaczony jako orientacyjny, z jasnym zastrzeżeniem, że ostateczna
  cena ustalana jest po bezpłatnej lekcji próbnej — bez fikcyjnych
  przekreślonych „cen promocyjnych”.
- Osobna sekcja na stronie głównej („Czego szczerze nie obiecujemy”) wprost
  odcina się od nierealistycznych obietnic typu „płynny angielski w 30 dni”.
